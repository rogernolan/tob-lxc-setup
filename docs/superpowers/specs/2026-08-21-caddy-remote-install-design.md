...
- The installer downloads the Caddy command from the public raw GitHub URL,
  validates the payload as the expected shell script, installs it atomically at
  `/usr/local/sbin/add-caddy-host`, and invokes it with the caller's arguments.
...
2. Download over HTTPS with `curl --fail --silent --show-error --location`,
   requiring TLS 1.2 or newer.
3. Validate that the temporary payload begins with the expected shell-script
   interpreter and contains the known command marker before installing it.
4. Install through a temporary file in `/usr/local/sbin`, then atomically move
   it into place with root ownership and mode `0755`.
...
- A review-first workflow that downloads the installer and command payloads to
  `/tmp`, displays them for inspection, then runs the installer.
...
- Existing fake-root test suite, extended for the installer.
